...
QKV_PREP = KernelScenario(
    ...
    description=(
        ...
        "defers it to whoever consumes the strided views. ONLY HALF OF THAT "
        "DEFERRAL IS COLLECTED. The value stays a strided view and its 4 MiB "
        "is timed in attention_core (scenario 5); the key stops being a view "
        "at k_layernorm, and qk_norm feeds its megatron arm a contiguous key, "
        "so the key's 4 MiB is timed in NO scenario today. The scenarios "
        "therefore sum to 4 MiB of this traffic, not 8, and this row read "
        "alone overstates titan's projection cost by roughly that traffic. The qk norms are "
...
QK_NORM = KernelScenario(
    ...
    description=(
        ...
        "attention_core times the value's half. The charge is not mirrored "
        ...
        "copy_floor moves the shared bytes and the x_floor column -- a ratio "
        "of times, unaffected by any bytes_moved -- decides whether the ratio "
        "is a kernel claim at all; the floor declares forward only, so "
