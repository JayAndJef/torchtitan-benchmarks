...
ATTN_RESIDUAL = KernelScenario(
    ...
    description=(
        ...
        "and no other arm declares bytes_moved, so no GB/s column reproduces "
        "it a second way. Suppressing comparisons removes the ROW and not the "
...
EXPERT_MLP = KernelScenario(
    ...
    description=(
        ...
        "declares no floor and no arm declares bytes_moved, so the table "
        "carries neither an x_floor nor a GB/s column, and neither of the two "
        "derived columns its sibling within-engine-only scenarios must "
        "caption exists here to reproduce the ratio a second way. Suppressing "
