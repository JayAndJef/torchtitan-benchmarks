        ...
        arms=(
            KernelArm(
                ...
                description=(
                    ...
                    "allgather dispatcher. All three, because token_combine "
                    "is guarded by tp_size > 1 or ep_size > 1 and is THE "
                    "IDENTITY at world_size=1 -- a cut on it alone would "
                    "measure a zero and read as a spectacular win -- and the "
                    "work sits in the neighbouring phases. The probabilities "
                    "are applied once, on the expert side of the cut"
                ),
                builder=(
                    "benchmarks.kernel.operations.expert_combine"
...
FFN_NORM_TO_MOE_RESIDUAL = KernelSpan(
    measurement=KernelScenario(
        ...
        description=(
            ...
            "tuple and hands the residual to mlp_bda. "
            "IT IS DELIVERED BY THE CONFIG FIELD ALONE, WITH NO SPEC KWARG, "
            "unlike expert_mlp's moe_grouped_gemm: "
            "transformer_engine_spec_provider.py:59-70 returns "
            "_TENormWithResidual for every has_residual norm whatever the "
            "flag says, so the spec never reads it. The one read site is "
            "TENorm.__new__, where the gate is two-level -- "
            "use_fused_residual = config.fused_residual_rmsnorm and "
            "has_residual "
            "(extensions/transformer_engine.py:1046) -- so with the flag off "
            "the same site builds a plain te.pytorch.RMSNorm. "
...
CHUNKED_CE = KernelSpan(
    measurement=KernelScenario(
        ...
        description=(
            ...
            "IT REPLACES THE SAME TWO ARMS fused_linear_ce REPLACES, so "
            "WITHIN ONE RUN the parts total is the same number in both "
            "files: the plan de-duplicates the shared range and measures it "
            "once. Across two runs it is two numbers, and the two spans are "
            "then not two numerators over one denominator. Each file records "
            "its own results_path, which is how a reader tells the two cases "
            "apart. "
            "THE SPAN-VERSUS-PARTS ROW CARRIES A BIAS AND IT FAVOURS THE "
            "SPAN: the parts total pays one host dispatch chain per enclosed "
            "scenario -- two here -- and the span pays one, and roughly 85% "
