...
one of them, because ``benchmarks.kernel.engine.run`` resolves every builder
by dotted path and calls them identically. Private helpers below take only
what they use. Timing closures and correctness runs use separate leaf
tensors so retained backward graphs are never disturbed.

``benchmarks.models.piper_qwen3.components.rope.te_rope_override`` is imported
only inside ``build_rope_te``: importing it JIT-builds the TE CUDA extension,
which needs the gcc-13 environment the runner injects for the rope scenario.
"""
...
from benchmarks.kernel.engine.run import BuiltArm
from benchmarks.kernel.registry import KernelWorkload
